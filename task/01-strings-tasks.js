...
function getRectangleString(width, height) {
    let hBorder = repeatString('─', width - 2);
    let rectBody = repeatString(`│${repeatString(' ',width - 2)}│\n`, height - 2);
    return `┌${hBorder}┐\n${rectBody}└${hBorder}┘\n`;
}
...
function encodeToRot13(str) {
    let firstCaseAlph = function(alph) {
        return (alph[0] === alph[0].toUpperCase() ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0));
    }
    return str.split("").map((s) => {
        if (!/[a-zA-Z]/.test(s[0]))
            return s[0];
        return String.fromCharCode((s.charCodeAt(0) - firstCaseAlph(s[0]) + 13) % 26 + firstCaseAlph(s[0]));
    }).join("");
}
...
function getCardId(value) {
    ...
    let numbPart = parseInt(value, 10);
    if (isNaN(numbPart))
        numbPart = decoder[value[0]];
    return (numbPart + decoder[value.slice(-1)] - 1);
}
